...
void Foam::immersedBoundaryFvPatch::makeInvDirichletMatrices() const
{
	...
	bool adaptive_ =ibmDict.lookupOrDefault("AdaptiveToStencil",true);
    ...
    forAll (idm, cellI)
    {
        ...
        if (adaptive_ && allPoints.size() < nCoeffs+3+10)
        {
			continue;
        }
        ...
        scalarField W =
            ...
            (
                1
              + cos(constant::mathematical::pi*curDist/(1.1*max(curDist)))
            );
        ...
        scalarRectangularMatrix lsM(nCoeffs, nCoeffs, 0.0);
        ...
        scalarRectangularMatrix invLsM = SVDinv(lsM,0.0); //more efficient matrix invert function in of30x
...
//Info<<"invDirichletMatrices conditionNumber "<<" "<<gMax(conditionNumber)<<" "<<gAverage(conditionNumber)<<endl;
}
...
void Foam::immersedBoundaryFvPatch::makeInvNeumannMatrices() const
{
    ...
    forAll (inm, cellI)
    {
		...
		{
			continue;
			/*else if(allPoints.size() < nCoeffs)
            {
				FatalErrorIn
		        (
		            "void immersedBoundaryFvPatch::makeInvDirichletMatrices()"
		        )   << "allPoints.size() < " << nCoeffs << " : "
		            << allPoints.size() << " at cell "<<ibc[cellI]<<" "<<C[ibc[cellI]]<<abort(FatalError);
			}*/
		}
        label pointID = 0;
        ...
        //scalarSquareMatrix lsM(nCoeffs, nCoeffs, 0.0);
        scalarRectangularMatrix lsM(nCoeffs, nCoeffs, 0.0);//more efficient matrix invert function in of30x
        for (label i = 0; i < lsM.n(); i++)
        {
            for (label j = 0; j < lsM.m(); j++)
            {
        ...
        // Calculate inverse
        //scalarSquareMatrix invLsM = LUinvert(lsM);
        scalarRectangularMatrix invLsM = SVDinv(lsM,0.0);
	...
	//Info<<"invNeumannMatrices conditionNumber "<<" "<<gMax(conditionNumber)<<" "<<gAverage(conditionNumber)<<endl;
}
