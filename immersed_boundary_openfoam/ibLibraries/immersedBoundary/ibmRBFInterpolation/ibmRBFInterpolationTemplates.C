...
template<class Type>
Foam::tmp<Foam::Field<Type> > Foam::ibmRBFInterpolation::interpolate
(
...
) const
{
    ...
    // Then, each CPU will do interpolation only on local ipPoints_

    if (ctrlField.size() != controlPoints_.size())
    {
        FatalErrorIn
        (
            "tmp<Field<Type> > ibmRBFInterpolation::interpolate\n"
            "(\n"
            "    const Field<Type>& ctrlField\n"
            ") const"
    ...
    (
        new Field<Type>(ipPoints_.size(), pTraits<Type>::zero)
    );
    ...
    forAll (ipPoints_, flPoint)
    {
            // Calculation of weights
            scalarField weights =
                RBF_->weights(controlPoints_, ipPoints_[flPoint]);
            ...
            if (polynomials_)
            {
                result[flPoint] +=
                    beta[0]
                  + beta[1]*ipPoints_[flPoint].x()
                  + beta[2]*ipPoints_[flPoint].y()
                  + beta[3]*ipPoints_[flPoint].z();
            }
    }
